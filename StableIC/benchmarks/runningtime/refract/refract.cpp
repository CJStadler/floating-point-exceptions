...
double computeApproxRefract(const double true_alt){
    double rval;
...
double reverse_refraction( const double true_alt)
{
   ...
   reval = computeApproxRefract(true_alt)
...
int main( const int argc, const char **argv)
{
   ...
   for(int i = 0; i < 5; i ++){
       ...
       ang = (double)0.2 * PI / 180;
       for( i = 1; i <= 100000; i++)
       {
              refraction( ang);
              reverse_refraction( ang);
              saasta_refraction( ang, 1013., 293., .20 );
              reverse_saasta_refraction( ang, 1013., 293., .20);
       }
       
       std::chrono::steady_clock::time_point end= std::chrono::steady_clock::now();
       
       if(std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count() < min){
           min = std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count();
       }
   }

   std::cout << "Time difference = " << min <<std::endl;
   return( 0);
}

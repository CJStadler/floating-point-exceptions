...
double mean_obliquity(double t_cen) __attribute__((always_inline))
{
    double u, u0;
    unsigned i;
...
void polar3_to_cartesian( double *vect, const double lon, const double lat) __attribute__((always_inline))
{
    double clat = cos( lat);
...
static int setup_ecliptic_precession_from_j2000(double year) __attribute__((always_inline))
{
...
int setup_precession(double year_from, double year_to) __attribute__((always_inline))
{
    double obliquity[2];
...
double triton( const double jd) __attribute__((always_inline))
{

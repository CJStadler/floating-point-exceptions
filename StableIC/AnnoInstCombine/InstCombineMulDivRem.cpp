...
Value *InstCombiner::foldFMulConst(Instruction *FMulOrDiv, Constant *C,
  ...
  if (FMulOrDiv->getOpcode() == Instruction::FMul) {
      ...
      if (isNormalFp(F)){
          ...
          //*** Attach delta_info to R ***
          Perturbation p;
          if(perturbation_map_.find(FMulOrDiv) != perturbation_map_.end()){
              p = perturbation_map_[FMulOrDiv];
              p.compositionMult(C, delta_builder_);
          }
          Constant *const_t = C1? C1 : C0;
          p.insertMDC(C1 ? Opnd0 : Opnd1, cast<ConstantFP>(const_t), cast<ConstantFP>(C), Instruction::FMul,
                      Instruction::FMul, true, InsertBefore, delta_builder_);
          perturbation_map_[R] = p;
          //******************************
    ...
    if (C0) {
      ...
      if (FMulOrDiv->hasOneUse()) {
          ...
          if (isNormalFp(F)){
              ...
              p.insertMDC(Opnd1, cast<ConstantFP>(C0), cast<ConstantFP>(C), Instruction::FDiv,
                          Instruction::FMul, true, InsertBefore, delta_builder_);
              perturbation_map_[R] = p;
              //******************************
          }
      ...
      if (isNormalFp(F)) {
        R = BinaryOperator::CreateFMul(Opnd0, F);
          ...
          p.insertMDC(Opnd0, cast<ConstantFP>(C), cast<ConstantFP>(C1), Instruction::FMul,
                      Instruction::FDiv, true, InsertBefore, delta_builder_);
          perturbation_map_[R] = p;
          //******************************
        ...
        Constant *F = ConstantExpr::getFDiv(C1, C);
          if (isNormalFp(F)){
              ...
              p.insertMDC(Opnd0, cast<ConstantFP>(C1), cast<ConstantFP>(C), Instruction::FDiv,
                          Instruction::FDiv, false, InsertBefore, delta_builder_);
              perturbation_map_[R] = p;
              //******************************
    ...
    if (AllowReassociate && isFiniteNonZeroFp(C)) {
      ...
      if (isFMulOrFDivWithConstant(Op0))
          if (Value *V = foldFMulConst(cast<Instruction>(Op0), C, &I)){
              //TODO: delete delta_info associated with I
      ...
      if (FAddSub &&
        ...
        if (C1 && isFiniteNonZeroFp(C1) && isFMulOrFDivWithConstant(Opnd0)) {
          ...
          if (M0 && M1) {
            ...
            //*** Attach delta_info to R ***
            Perturbation p;
            if(perturbation_map_.find(FAddSub) != perturbation_map_.end()){
                  p = perturbation_map_[FAddSub];
                  p.compositionMult(C, delta_builder_);
            }
            p.insertMulDivideConst(cast<ConstantFP>(C1), cast<ConstantFP>(C), &I, delta_builder_);
            perturbation_map_[RI] = p;
            //******************************
...
static Instruction *CvtFDivConstToReciprocal(Value *Dividend, Constant *Divisor,
                                             bool AllowReciprocal, PerturbationMap& perturbation_map_,
                                             IRBuilder<> &delta_builder_, Instruction* I) {
  ...
  Instruction *Res = BinaryOperator::CreateFMul(Dividend, R);
    
    //*** Attach delta_info to R ***
    if(inexact){
        Perturbation p;
        p.insertReciprocal(Dividend, cast<ConstantFP>(Divisor), I, delta_builder_);
        perturbation_map_[Res] = p;
    }
    //******************************
  ...
  if (Constant *Op1C = dyn_cast<Constant>(Op1)) {
    ...
    if (AllowReassociate) {
      ...
      if (match(Op0, m_FMul(m_Value(X), m_Constant(C1)))) {
        ...
        Constant *C = ConstantExpr::getFDiv(C1, C2);
          if (isNormalFp(C)){
              ...
              if(perturbation_map_.find(Op0) != perturbation_map_.end()){
                  ...
                  Constant *reciC2 = ConstantExpr::getFDiv(ConstantFP::get(X -> getType(), 1.0f), C2);
                  p.compositionMult(reciC2, delta_builder_);
              }
              p.insertMDC(X, cast<ConstantFP>(C1), cast<ConstantFP>(C2),
                          Instruction::FMul, Instruction::FDiv, true, &I, delta_builder_);
              perturbation_map_[Res] = p;
              //******************************
          }
        ...
        if (isNormalFp(C)) {
          Res = CvtFDivConstToReciprocal(X, C, AllowReciprocal, perturbation_map_, delta_builder_, &I);
            if (!Res){
                ...
                p.insertMDC(X, cast<ConstantFP>(C1), cast<ConstantFP>(C2),
                            Instruction::FDiv, Instruction::FMul, false, &I, delta_builder_);
                perturbation_map_[Res] = p;
                //******************************
            }
        }
    ...
    // X / C => X * 1/C
    if (Instruction *T = CvtFDivConstToReciprocal(Op0, Op1C, AllowReciprocal,perturbation_map_, delta_builder_, &I)) {
  ...
  if (AllowReassociate && isa<Constant>(Op0)) {
    ...
    if (match(Op1, m_FMul(m_Value(X), m_Constant(C2)))){
        ...
        if(perturbation_map_.find(Op1) != perturbation_map_.end()){
            pOp1 = perturbation_map_[Op1];
            Constant *recipC2 = ConstantExpr::getFDiv(ConstantFP::get(X -> getType(), 1.0f), C2);
            pOp1.compositionMult(recipC2, delta_builder_);
        }
        p.insertMDC(X, cast<ConstantFP>(C1), cast<ConstantFP>(C2),
                    Instruction::FDiv, Instruction::FDiv, true, &I, delta_builder_);
      ...
      // C1 / (X/C2) => (C1*C2) / X
        ...
        if(perturbation_map_.find(Op1) != perturbation_map_.end()){
            pOp1 = perturbation_map_[Op1];
            pOp1.compositionMult(C2, delta_builder_);
        }
        
        p.insertMDC(X, cast<ConstantFP>(C1), cast<ConstantFP>(C2), Instruction::FDiv,
      ...
      if(perturbation_map_.find(Op1) != perturbation_map_.end()){
          pOp1 = perturbation_map_[Op1];
          BinaryOperator *bo1 = BinaryOperator::CreateFMul(X, X);
          BinaryOperator *bo2 = BinaryOperator::CreateFDiv(C2, bo1);
          pOp1.compositionMult(bo2, delta_builder_);
      }
        
      p.insertMDC(X, cast<ConstantFP>(C1), cast<ConstantFP>(C2), Instruction::FMul,
                  Instruction::FDiv, true, &I, delta_builder_);
    }
    ...
    if (Fold && isNormalFp(Fold)) {
      ...
      if(pOp1.getDeltas().size() > 0){
            perturbation_map_[X] = pOp1;
      }
      perturbation_map_[R] = p;
      return R;
    }

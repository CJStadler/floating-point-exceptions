...
static cl::opt<bool>
EnableExpensiveCombines("anno-expensive-combines",
                        cl::desc("Enable expensive instruction combines"));

static cl::opt<unsigned>
MaxArraySize("anno-instcombine-maxarray-size-output", cl::init(1024),
...
bool InstCombiner::SimplifyAssociativeOrCommutative(BinaryOperator &I) {
    ...
    do {
        ...
        if (I.isAssociative()) {
            DEBUG(dbgs() << "IC: SimplifyAssociativeOrCommutative: " << I << '\n');
            ...
            if (Op0 && Op0->getOpcode() == Opcode) {
                ...
                if (Value *V = SimplifyBinOp(Opcode, B, C, SQ.getWithInstruction(&I))) {
                    ...
                    //************* Attach delta info into I
                    Perturbation p;
                    if(perturbation_map_.find(Op0) != perturbation_map_.end()){
                        p = perturbation_map_[Op0];
                    }
                    
                    if(Opcode == Instruction::FAdd){
                        p.insertConstCommAndAsso(A, cast<ConstantFP>(B), cast<ConstantFP>(C), &I, delta_builder_);
                        
                    }else if(Opcode == Instruction::FMul){
                        p.compositionMult(cast<ConstantFP>(C), delta_builder_);
                        p.insertMDC(A, cast<ConstantFP>(B), cast<ConstantFP>(C), Instruction::FMul,
                                    Instruction::FMul, true, &I, delta_builder_);
                    }
                    perturbation_map_[&I] = p;
                    //**************************************
            ...
            if (Op1 && Op1->getOpcode() == Opcode) {
                ...
                if (Value *V = SimplifyBinOp(Opcode, A, B, SQ.getWithInstruction(&I))) {
                    ...
                    //************* Attach delta info into I
                    Perturbation p;
                    if(perturbation_map_.find(Op1) != perturbation_map_.end()){
                        p = perturbation_map_[Op1];
                    }
                    if(Opcode == Instruction::FAdd){
                        p.insertConstCommAndAsso(C, cast<ConstantFP>(B), cast<ConstantFP>(A), &I, delta_builder_);
                    }else if(Opcode == Instruction::FMul){
                        p.compositionMult(cast<ConstantFP>(A), delta_builder_);
                        p.insertMDC(C, cast<ConstantFP>(B), cast<ConstantFP>(A),
                                    Instruction::FMul, Instruction::FMul, true, &I, delta_builder_);
                    }
                    perturbation_map_[&I] = p;
                    //**************************************
        ...
        if (I.isAssociative() && I.isCommutative()) {
            ...
            if (Op0 && Op0->getOpcode() == Opcode) {
                ...
                // Does "C op A" simplify?
                 if (Value *V = SimplifyBinOp(Opcode, C, A, SQ.getWithInstruction(&I))) {
                    ...
                    if(Opcode == Instruction::FAdd){
                        p.insertConstCommAndAsso(B, cast<ConstantFP>(A), cast<ConstantFP>(C), &I, delta_builder_);
                    }else if(Opcode == Instruction::FMul){
                        p.compositionMult(cast<ConstantFP>(C),delta_builder_);
                        p.insertMDC(B, cast<ConstantFP>(A), cast<ConstantFP>(C),
                                    Instruction::FMul, Instruction::FMul, true, &I, delta_builder_);
                    }
                    perturbation_map_[&I] = p;
                    //**************************************
            ...
            if (Op1 && Op1->getOpcode() == Opcode) {
                ...
                if (Value *V = SimplifyBinOp(Opcode, C, A, SQ.getWithInstruction(&I))) {
                    ...
                    if(Opcode == Instruction::FAdd){
                        p.insertConstCommAndAsso(B, cast<ConstantFP>(C), cast<ConstantFP>(A), &I, delta_builder_);
                    }else if(Opcode == Instruction::FMul){
                        p.compositionMult(cast<ConstantFP>(A), delta_builder_);
                        p.insertMDC(B, cast<ConstantFP>(C), cast<ConstantFP>(A),
                                    Instruction::FMul, Instruction::FMul, true, &I, delta_builder_);
                    }
                    perturbation_map_[&I] = p;
                    //**************************************
            ...
            if (Op0 && Op1 &&
                ...
                if(Opcode == Instruction::FAdd){
                    ...
                    if(perturbation_map_.find(Op1) != perturbation_map_.end()){
                        p.compositionAdd(perturbation_map_[Op1], delta_builder_);
                    }
                    p.insertConstCommAndAsso(New, cast<ConstantFP>(C1), cast<ConstantFP>(C2), &I, delta_builder_);
                }else if(Opcode == Instruction::FMul){
                    Perturbation p1, p2;
                    if(perturbation_map_.find(Op0) != perturbation_map_.end()){
                        p1 = perturbation_map_[Op0];
                        BinaryOperator *NewBC2 = cast<BinaryOperator>(delta_builder_.CreateFMul(B, C2));
                        p1.compositionMult(NewBC2, delta_builder_);
                        p.compositionAdd(p1, delta_builder_);
                    }
                    
                    if(perturbation_map_.find(Op1) != perturbation_map_.end()){
                        p2 = perturbation_map_[Op1];
                        BinaryOperator *NewAC1 = cast<BinaryOperator>(delta_builder_.CreateFMul(A, C1));
                        p2.compositionMult(NewAC1, delta_builder_);
                        p.compositionAdd(p2, delta_builder_);
                    }
                    
                    p.insertMDC(New, cast<ConstantFP>(C1), cast<ConstantFP>(C2),
                                Instruction::FMul, Instruction::FMul, true, &I, delta_builder_);
                }
                perturbation_map_[&I] = p;
                //**************************************
...
combineInstructionsOverFunction(Function &F, InstCombineWorklist &Worklist,
                                ...
                                TargetLibraryInfo &TLI, DominatorTree &DT, PerturbationMap &perturbation_map,
    ...
    BasicBlock *deltaBlock = BasicBlock::Create(F.getContext());
    IRBuilder<> delta_builder(deltaBlock);
    ...
    for (;;) {
        ...
        InstCombiner IC(Worklist, Builder, F.optForMinSize(), ExpensiveCombines,
                        AA, AC, TLI, DT, DL, LI, perturbation_map, delta_builder);
   ...
   // mod->print(errs(), nullptr);
    //deltaBlock->insertInto(&F);
    
    return MadeIRChange || Iteration > 1;
}

PreservedAnalyses AnnoInstCombinePass::run(Function &F,
    ...
    if (!combineInstructionsOverFunction(F, Worklist, nullptr, AC, TLI, DT, perturbation_map_,
...
void AnnoInstructionCombiningPass::getAnalysisUsage(AnalysisUsage &AU) const {
    AU.setPreservesCFG();
...
bool AnnoInstructionCombiningPass::runOnFunction(Function &F) {
    //if (skipFunction(F))
    //return false;
    // Required analyses.
    ...
    return combineInstructionsOverFunction(F, Worklist, AA, AC, TLI, DT, perturbation_map_,
                                           ExpensiveCombines, LI);
}

char AnnoInstructionCombiningPass::ID = 0;

static RegisterPass<AnnoInstructionCombiningPass> X("annoinstcombine", "Combine redundant instructions and annotate them");

// INITIALIZE_PASS_BEGIN(AnnoInstructionCombiningPass, "annoinstcombine",
//                       "Combine redundant instructions and annotate them", false, false)
// INITIALIZE_PASS_DEPENDENCY(AssumptionCacheTracker)
// INITIALIZE_PASS_DEPENDENCY(TargetLibraryInfoWrapperPass)
// INITIALIZE_PASS_DEPENDENCY(DominatorTreeWrapperPass)
// INITIALIZE_PASS_DEPENDENCY(AAResultsWrapperPass)
// INITIALIZE_PASS_DEPENDENCY(GlobalsAAWrapperPass)
// INITIALIZE_PASS_END(AnnoInstructionCombiningPass, "annoinstcombine",
//                     "Combine redundant instructions and annotate them", false, false)
...
FunctionPass *llvm::createInstructionCombiningPass(bool ExpensiveCombines) {
    return new AnnoInstructionCombiningPass(ExpensiveCombines);
}
